...
namespace ModelBinding.Models
{
    public class Employee
    {
        ...
        public static List<SelectListItem> GetDepts()
        {
            //List<Employee> lstEmps = new List<Employee>();
            List<SelectListItem> lstDeps = new List<SelectListItem>();
            SqlConnection cn = new SqlConnection();
            cn.ConnectionString = @"Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=KtJune23;Integrated Security=True";
            try
            {
                ...
                cmdInsert.Connection = cn;
                cmdInsert.CommandType = System.Data.CommandType.Text;
                cmdInsert.CommandText = "select * from Departments ";
                SqlDataReader dr = cmdInsert.ExecuteReader();
                while (dr.Read())
                {
                    //lstEmps.Add(new Employee { EmpNo = dr.GetInt32(0), Name = dr.GetString(1), Basic = dr.GetDecimal(2), DeptNo = dr.GetInt32(3) });
                    lstDeps.Add(new SelectListItem { Text = dr.GetString("DeptName"), Value = dr["DeptNo"].ToString() });
                }
                dr.Close();
            }
            catch (Exception ex)
